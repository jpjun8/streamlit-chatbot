...
if prompt := st.chat_input("How would you like to reply?"):
    ...
    with st.spinner('CoPilot is thinking...'):
        # Create a chat message box for diplaying the assistant's response
        with st.chat_message("assistant"):
            # Initialize an empty string to construct the full respone incrementally
            full_response = ""

            # Create an empty placeholder to stream the assistant's response
            message_placeholder = st.empty()

            # Loop through the respone generator
            for response in response_generated:
                ...
                # Break the content into chunk of 10 words each
